...
locals {
  ecommerce_distribution_origin_id = "${aws_api_gateway_domain_name.api_blue.regional_domain_name}-origin"
}
...
resource "aws_cloudfront_distribution" "ecommerce_distribution" {
  origin {
    ...
    origin_id   = local.ecommerce_distribution_origin_id
  ...
  default_cache_behavior {
    ...
    target_origin_id         = local.ecommerce_distribution_origin_id
...
data "aws_wafv2_web_acl" "cloudfront_managed" {
  name  = "CreatedByCloudFront-37462c05"
  scope = "CLOUDFRONT"
}
