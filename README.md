...
* The Lambda functions in my project are built in .NET. That is my language of choice, but the concepts in this project are applicable to any language supported by Lambda.
...
* [src/Ecommerce/Ecommerce.Library.Api.Tests](src/Ecommerce/Ecommerce.Library.Api.Tests) is a regular .NET unit test project. This represents _any_ test suite that confirm the functionality of the API.
    * I kept it the same language as the API project for simplicity.
    * Instead of or in addition to automated tests, you could also do manual testing of the blue stage before promotion, with something like a [Manual Workflow Approval](https://github.com/marketplace/actions/manual-workflow-approval) as a gate before promotion.
